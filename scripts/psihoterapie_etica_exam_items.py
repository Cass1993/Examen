"""Itemi examen — Psihoterapie: etică, psihodramă și terapia unificării."""
...
LOT_NAME = "Psihoterapie: etică, psihodramă și terapia unificării"
START_ID = 7301
...
def _load_raw():
    ...
    data_path = (
        Path(__file__).resolve().parent.parent / "data" / "psihoterapie_etica_items.json"
    )
    if data_path.exists():
        return json.loads(data_path.read_text(encoding="utf-8"))
    from scripts.psihoterapie_etica_bank_data import ITEMS

    return ITEMS


def build_items() -> List[Dict]:
    from scripts.psihoterapie_etica_option_polish import polish_bank_row
